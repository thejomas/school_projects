...
let rec evalExp (e : UntypedExp, vtab : VarTable, ftab : FunTable) : Value =
  ...
  | And (e1, e2, pos) ->
        ...
        match res1 with
          ...
          | BoolVal true  ->
            ...
            match res2 with
              | BoolVal b  -> BoolVal b
              | _ -> invalidOperand "&& on non-boolean args: " Bool res2 pos
          | _ -> invalidOperand "&& on non-boolean args: " Bool res1 pos
  ...
  | Negate(e, pos) ->
        ...
        match res with
          | IntVal n -> IntVal (-n)
          | _ -> invalidOperand "~ on non-integral args: " Int res pos
  ...
  | Apply("length", [arg], pos) ->
        ...
        match evarg with
          ...
          | _ -> raise (MyError("Call length function on non-array arg"+ppVal 0 evarg, pos))
  ...
  | Map (farg, arrexp, _, _, pos) ->
        ...
        match arr with
          ...
          | _ -> raise (MyError( "Second argument of map is not an array: "+ppVal 0 arr
                                       , pos))
  | Reduce (farg, ne, arrexp, _, pos) ->
        ...
        match arr with
          ...
          | _ -> raise (MyError("Third argument of reduce is not an array: "+ppVal 0 arr
                                       , pos))
  ...
  | Replicate (e1, e2, _, pos) ->
        ...
        match res1 with
          | IntVal n when (n >= 0) ->
            let mlst = List.replicate n res2
            ArrayVal (mlst, valueType res2)
          | _ -> raise (MyError( "First argument of replicate is not an integer: "+ppVal 0 res1
                                       , pos))

  (* DONE project task 2: `filter(p, arr)` *)
        ...
        match arr with
          | ArrayVal (lst,tp1) ->
               let mlst = List.filter (fun x ->
                  ...
                  match res with
                    | BoolVal b -> b
                    | _ -> raise (MyError( "Return type of function is not an bool: "+ppVal 0 res
                                       , pos))) lst
               ArrayVal (mlst, tp1)
          | _ -> raise (MyError( "Second argument of filter is not an array: "+ppVal 0 arr
                                       , pos))
  ...
  | Scan (farg, ne, arrexp, _, pos) ->
        ...
        match arr with
          | ArrayVal (lst, tp1) ->
               ...
               ArrayVal (List.tail mlst, tp1)
          | _ -> raise (MyError("Third argument of Scan is not an array: "+ppVal 0 arr
                                       , pos))
  ...
  | Read (t,p) ->
        ...
        match t with
          ...
          | Char -> let v : char = char str
                    CharVal v
          | _ -> raise (MyError("Read operation is valid only on basic types ", p))
  ...
  | Write(exp,t,p) ->
        ...
        match v with
          ...
          | ArrayVal (a, Char) ->
             let mapfun = function
                   | CharVal c -> c
                   | _ -> raise (MyError("Write argument " +
                                                   ppVal 0 v +
                                                   " should have been evaluated to string", p))
             printfn "%s" ( System.String.Concat (List.map mapfun a) )
          | _ -> raise (MyError("Write can be called only on basic and array(char) types ", p))
        v
...
and callFunWithVtable (fundec : UntypedFunDec
    ...
    match fid with
      ...
      | "ord" -> match aargs with
                   ...
                   | _ -> raise (MyError("Argument of \"ord\" does not evaluate to Char: "+
                                                  (String.concat "" (List.map (ppVal 0) aargs)), pcall))
      | "chr" -> match aargs with
                    | [IntVal n] -> CharVal (char n)
                    | _ -> raise (MyError("Argument of \"chr\" does not evaluate to Num: "+
                                               (String.concat "" (List.map (ppVal 0) aargs)), pcall))
