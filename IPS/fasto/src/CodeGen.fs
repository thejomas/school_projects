...
let mipsStore (elem_size : ElemSize) =
    match elem_size with
      | One  -> Mips.SB
      | Four -> Mips.SW

let mipsLoad (elem_size : ElemSize)  =
    match elem_size with
      | One  -> Mips.LB
      | Four -> Mips.LW
...
let rec compileExp  (e      : TypedExp)
  ...
  | Constant (BoolVal b, pos) ->
      match b with
        | true  -> [Mips.LUI (place, makeConst 1)]
        | false -> [Mips.LUI (place, makeConst 0)]
  ...
  *)
  | Iota (n_exp, (line, _)) ->
      ...
      *)
      ...
      let loop_body   = [ Mips.SW (i_reg, addr_reg, "0") ]
  ...
  | Map (farg, arr_exp, elem_type, ret_type, pos) ->
      let elemType = getElemSize elem_type
      let retType  = getElemSize ret_type
      let elemSize = makeConst (elemSizeToInt elemType)
      let retSize  = makeConst (elemSizeToInt retType)
      let load     = mipsLoad elemType  (* MIPS Load instruction *)
      let store    = mipsStore retType  (* MIPS Store instruction *)
      ...
      let loop_body =
          load (tmp_reg, arr_reg, "0")
          :: applyFunArg(farg, [tmp_reg], vtable, tmp_reg, pos)
          @ [ store (tmp_reg, addr_reg, "0") ]

      let loop_footer =
          [ Mips.ADDI (i_reg, i_reg, "1")
          ; Mips.ADDI (arr_reg, arr_reg, elemSize)
          ; Mips.ADDI (addr_reg, addr_reg, retSize)
          ; Mips.J loop_beg
          ; Mips.LABEL loop_end ]
  ...
  | Reduce (farg, acc_exp, arr_exp, elem_type, pos) ->
      let elemType = getElemSize elem_type
      let elemSize = makeConst (elemSizeToInt elemType)
      let load     = mipsLoad elemType   (* MIPS Load instruction *)
  ...
  | Replicate (n_exp, a_exp, elem_type, (line, _)) ->
      let elemType = getElemSize elem_type
      let elemSize = makeConst (elemSizeToInt elemType)
      let store    = mipsStore elemType (* MIPS Store instruction *)
      let size_reg = newName "size_reg" (* size of output array *)
      let elem_reg = newName "elem_reg" (* address of input element *)
      let safe_lab = newName "safe_lab"
      let addr_reg = newName "addr_reg" (* address of element in new array *)
      ...
      let loop_body = [ store (elem_reg, addr_reg, "0") ]

      let loop_footer =
          [ Mips.ADDI (i_reg, i_reg, "1")
          ; Mips.ADDI (addr_reg, addr_reg, elemSize)
          ; Mips.J loop_beg
          ; Mips.LABEL loop_end ]
  ...
  | Filter (farg, arr_exp, elem_type, pos) ->
      ...
      let store      = mipsStore elemType (* MIPS Store instruction *)
      let size_reg   = newName "size_reg" (* size of input array *)
      let arr_reg    = newName "arr_reg"  (* address of input array / element *)
      let res_reg    = newName "res_reg"
      let addr_reg   = newName "addr_reg" (* address of element in new array *)
      let i_reg      = newName "i_reg"    (* iterator for input array *)
      let j_reg      = newName "j_reg"    (* iterator for result array *)
      let loop_beg   = newName "loop_beg"
      let loop_end   = newName "loop_end"
      let tmp_reg    = newName "tmp_reg"
      let falseLabel = newName "false"
  ...
  | Scan (farg, acc_exp, arr_exp, elem_type, pos) ->
      ...
      let size_reg = newName "size_reg" (* size of input/output array *)
      let arr_reg  = newName "arr_reg"  (* address of input array / element *)
      let acc_reg  = newName "acc_reg"  (* address of accumulator /result  *)
      let addr_reg = newName "addr_reg" (* address of element in new array *)
      let i_reg    = newName "i_reg"
      let loop_beg = newName "loop_beg"
      let loop_end = newName "loop_end"
